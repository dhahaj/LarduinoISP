...
void empty_reply(void) {
  if (CRC_EOP == getch()) {
    debug_print((char)STK_INSYNC);
    debug_print((char)STK_OK);
  }
  else {
    error++;
    debug_print((char)STK_NOSYNC);
  }
}
...
void write_flash(int length) {
  ...
  if (CRC_EOP == getch()) {
    debug_print((char) STK_INSYNC);
    debug_print((char) write_flash_pages(length));
  }
  else {
    error++;
    debug_print((char) STK_NOSYNC);
...
char flash_read_page(int length) {
  for (int x = 0; x < length; x += 2) {
    ...
    debug_print((char) (word & 0xff));
    debug_print((char) ((word >> 8) & 0xff));
    address++;
  }
  return STK_OK;
}
...
void read_signature(void) {
  ...
  debug_print((char) STK_INSYNC);
  debug_print((char) 0x1e);
  debug_print((char) 0x95);
  debug_print((char) 0x0f);
  debug_print((char) STK_OK);
}
